...
T.PP = function(p, obj)
	...
	elseif p == 6 then
		...
		obj:SetPoint('BOTTOM', right)
	elseif p==9 then									--
		obj:SetParent(TukuiInfocentre)					--	
		obj:SetHeight(left:GetHeight())					--
		obj:SetPoint("LEFT", left, 570, 0)				--
		obj:SetPoint('TOP', left,0,3)
		obj:SetPoint('BOTTOM', left)
	elseif p==10 then
		obj:SetParent(TukuiInfocentre)
		obj:SetHeight(left:GetHeight())					-- added panel under the action bars for 3 additional datatext
		obj:SetPoint("LEFT", left, 720, 0)
		obj:SetPoint('TOP', left,0,3)
		obj:SetPoint('BOTTOM', left)
	elseif p==11 then
		obj:SetParent(TukuiInfocentre)
		obj:SetHeight(left:GetHeight())					--
		obj:SetPoint("LEFT", left, 855, 0)				--
		obj:SetPoint('TOP', left,0,3)					--
		obj:SetPoint('BOTTOM', left)					--
	end
...
T.HidePortrait = function(self, unit)
	if self.unit == "target" then
		if not UnitExists(self.unit) or not UnitIsConnected(self.unit) or not UnitIsVisible(self.unit) then
			...
			if not (C["unitframes"].portraitonuf == true) then
