...
--remover backdrop because of the reducer unitframes size

	-- this is the glow border
--removed glow border because it was too large for my unitframes
	...
	if (unit == "player" or unit == "target") then
		...
		if T.lowversion then
			panel:CreatePanel("Default", 188, 31, "BOTTOM", self, "BOTTOM", 0, 22)
		else
		...
		if T.lowversion then
			health:Height(22)
			panelhp:CreatePanel("Default", 186, 5, "BOTTOMLEFT", panel, "BOTTOMLEFT", 0, 0)
			panelhp:SetFrameLevel(3)
		else
			health:Height(26)
			panelhp:CreatePanel("Default", 250, 26, "TOPLEFT", panel, "TOPLEFT", 0, 0)
		end
		...
		local anchorPanel = CreateFrame("Frame", anchorPanel, icentre)
		...
		if (C["unitframes"].unitcastbar == true) then
			...
			if unit == "player" then											--
				castbar:Height(T.Scale(C.actionbar.buttonsize)-8)
				castbar:Width(T.InfoLeftRightWidth+4)
				castbar:SetPoint("TOPLEFT", TukuiBar1, "BOTTOMLEFT", 2, -3)		--surce adeed to separate the castbar of player and target
			elseif unit == "target" then
				castbar:Height(T.Scale(C.actionbar.buttonsize))
				castbar:Width(450)
			...
			if C["unitframes"].cbicons == true then
				...
				elseif unit == "target" then
						...
						castbar.button:SetPoint("RIGHT", -215, 40)
	...
	if (unit == "targettarget") then
		...
		local panel = CreateFrame("Frame", nil, self)
		panel:CreatePanel("Default", 188, 21, "BOTTOM", self, "BOTTOM", 0, -1)
		
		if not T.lowversion then
			panel:CreatePanel("Default", 131, 17, "BOTTOM", self, "BOTTOM", 0, T.Scale(0))
			panel:SetFrameLevel(2)
			panel:SetFrameStrata("MEDIUM")
			panel:SetBackdropBorderColor(unpack(C["media"].altbordercolor))
			self.panel = panel
...
player:SetPoint("BOTTOMLEFT", InvTukuiActionBarBackground, "TOPLEFT", 0,-13+adjustXY)
if T.lowversion then
	player:Size(186, 51)
else
	player:Size(250, 57)
...
target:SetPoint("BOTTOMRIGHT", InvTukuiActionBarBackground, "TOPRIGHT", 0,-13+adjustXY)
if T.lowversion then
	target:Size(186, 51)
else
	target:Size(250, 57)
